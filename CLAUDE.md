...
1. Todo vendor externo tem adapter com interface própria + `fake.ts`. Nunca importe SDK de vendor fora de `adapters/`. Exceção registrada: bibliotecas de processamento local sem rede (`sharp`, `next/og`, `qrcode`) podem viver em `services/` e em `src/receipts/`; vendor com rede continua só em `adapters/`.
