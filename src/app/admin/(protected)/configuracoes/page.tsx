...
import { loadStudioSettings, loadStudioVideoSettings, type StudioSettings, type StudioVideoSettings } from "@/services/studio-settings";
...
type SettingsData = {
  ...
  /** Vídeo da peça: o teto de vídeos por dia (o interruptor fica em Vendedora & WhatsApp). */
  studioVideo: StudioVideoSettings;
...
async function loadSettings(): Promise<SettingsData | null> {
  try {
    const db = getDb();
    const [feeRules, policy, map, storeFactsPreview, studio, studioVideo] = await Promise.all([
      getFeeRules(db),
      getDefaultPolicy(db),
      getSettingsMap(db, [
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Card
        id="foto-no-corpo"
        title="Foto no corpo"
        description="O ensaio das peças no corpo das modelos da casa: quantas imagens e vídeos por dia, a qualidade e a combinação que o formulário da peça já traz preenchida. Liga e desliga em Vendedora & WhatsApp."
      >
        <StudioSettingsForm
          defaults={{
            ...
            videoDailyLimit: data.studioVideo.dailyLimit,
          }}
