...
export default async function EditionCardsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      {/* Na impressão só os cartões aparecem, um por página: a casca do painel some. */}
      <style>{`@media print { body * { visibility: hidden !important; } .edition-print, .edition-print * { visibility: visible !important; } .edition-print { position: absolute; left: 0; top: 0; margin: 0; } .edition-card { break-after: page; page-break-after: always; } @page { margin: 1cm; } }`}</style>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Um cartão por peça, 9 × 12 cm. Imprima em papel de gramatura alta, corte e coloque dentro da caixa.
            {generated && cards.at ? ` Gerados em ${formatDateTimeSP(cards.at)}.` : ""}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <CardsForm orderId={id} generated={generated} />
          {generated ? <PrintButton label="Imprimir cartões" /> : null}
        </div>
      </div>

      {cards.cards.length === 0 ? (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Este pedido não tem peças.</p>
      ) : (
        <ul className="edition-print flex flex-col gap-8">
          {cards.cards.map((card) => (
            <li key={card.productId} className="edition-card flex flex-col items-center gap-2">
              {card.url ? (
                <img
                  src={card.url}
              ...
              <p className="text-xs text-zinc-500 print:hidden dark:text-zinc-400">
                {card.name}
                {card.hasCuratorNote ? "" : " · sem frase da curadora (grave a nota na tela da peça e gere de novo)"}
              </p>
            </li>
          ))}
        </ul>
