...
import { spDayLabel, spNextDayKey, spWeekdayName } from "@/lib/sp-day";
...
/** Hoje e os próximos 3 dias × cada janela ativa: as opções do "Reagendar". */
function rescheduleChoices(todayKey: string, rates: { rateName: string; windows: { start: string; end: string; cutoff: string }[] }[]): RescheduleChoice[] {
  const choices: RescheduleChoice[] = [];
  let dayKey = todayKey;
  for (let i = 0; i < 4; i += 1) {
    ...
    for (const rate of rates) {
      for (const w of rate.windows) {
        choices.push({
          value: `${dayKey}|${w.start}|${w.end}|${w.cutoff}`,
          label: `${dayLabel}, ${hourLabel(w.start)}–${hourLabel(w.end)}${rates.length > 1 ? ` · ${rate.rateName}` : ""}`,
        });
      }
    }
      ...
      {route.late.length > 0 ? (
        <section className="flex flex-col gap-3 rounded-lg border border-red-200 bg-red-50/40 p-4 dark:border-red-900 dark:bg-red-950/20">
          <h2 className="text-sm font-semibold tracking-wide text-red-800 uppercase dark:text-red-300">
            Atrasados <span className="font-normal text-red-700/70 dark:text-red-300/70">· {route.late.length} — a janela passou e a peça não saiu</span>
          </h2>
          <ul className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
