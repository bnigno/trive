...
// (/pedido/<token>, /lancamento/<token>, /v/<token> da votação) e cupons
// (?cupom=, /c/<código>) nunca saem do navegador: a rota vira /pedido,
// /lancamento, /v ou /c.
export function anonymizeAnalyticsUrl(rawUrl: string): string {
  ...
  if (/^\/v\/[^/]+/.test(url.pathname)) url.pathname = "/v";
