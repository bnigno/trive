...
export async function execCotarFrete(
  ...
  const options = await quoteDeliveryOptions(db, { cep: input.cep, totalWeightGrams, now }, { correios: ctx.correiosQuoter });
  ...
  if (quotes.some((quote) => quote.kind === "correios")) {
    lines.push("[Correios: o prazo é em dias úteis a partir da postagem e o valor já inclui a embalagem. A cliente escolhe PAC ou SEDEX; passe a escolha em criar_pedido (campo frete).]");
  }
  if (isEstimate) {
