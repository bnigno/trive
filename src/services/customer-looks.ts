 ...
 * mensagem com a foto e a fila faz o resto. Devolve o id da foto registrada.
 */
 ...
 * a reentrada não manda de novo). Foto que a Z-API não entrega mais fica
 * sem cartão — a linha continua, sem consentimento, nunca pública.
 */
export async function renderAndSendCustomerLookCard(
  ...
  if (!photoPath) {
    ...
    try {
      ...
      jpeg = await normalizeLookPhoto(media.data);
    } catch (error) {
      console.warn("[customer-looks] foto da cliente não baixou:", error instanceof Error ? error.message : error);
      return { skipped: "foto_indisponivel" };
    }
  ...
  if (!cardPath) {
    ...
    try {
      photoDataUrl = await loadCardPhotoDataUrl(storage, photoPath, cardFrameSize("customer_look", "hero", 1));
    } catch (error) {
      if (error instanceof PhotoUnavailableError) return { skipped: "foto_indisponivel" };
      throw error;
    }
...
export type LookConsentContext = { lookId: string; productName: string; answer: LookConsentAnswer };

/**
 * O toque chegou (webhook): grava a resposta — só do telefone que recebeu a
 * pergunta e só a primeira vez — e enfileira a confirmação curta. Devolve o
 * contexto para o histórico; null = nada gravado (telefone diferente,
 * segunda resposta, foto retirada).
 */
export async function recordLookConsent(
  ...
  const [look] = await tx
    ...
    .limit(1);
  if (!look || look.phoneE164 !== input.phoneE164 || look.consentAnswer !== null || look.revokedAt) return null;
  const updated = await tx
    ...
    .where(and(eq(customerLooks.id, input.lookId), isNull(customerLooks.consentAnswer)))
    .returning({ id: customerLooks.id });
  if (updated.length === 0) return null;
  const dedupeKey = `wa.look_consent_ack:${input.lookId}`;
  await enqueueOutboxEvent(tx, {
    eventType: "wa.send",
    dedupeKey,
    aggregateType: "customer_look",
    aggregateId: input.lookId,
    payload: { templateKey: null, phoneE164: input.phoneE164, body: lookConsentAckText(input.answer), dedupeKey },
  });
  return { lookId: input.lookId, productName: look.productName, answer: input.answer };
}
...
/** Retirada de uma foto (dona ou sistema): some da vitrine para sempre. */
 ...
 * as fotos do telefone saem (as que estavam na vitrine e as que só esperavam).
 * `wasPublic` = quantas estavam na página de fato.
 */
export async function revokeCustomerLooksByPhone(
  db: DbOrTx,
  input: { phoneE164: string; source: "customer" | "lia" | "forget"; now?: Date },
): Promise<{ revoked: number; wasPublic: number }> {
  const now = input.now ?? new Date();
  const updated = await db
    .update(customerLooks)
    .set({ revokedAt: now, revokedBy: input.source, updatedAt: now })
    .where(and(eq(customerLooks.phoneE164, input.phoneE164), isNull(customerLooks.revokedAt)))
    .returning({ id: customerLooks.id, consentAnswer: customerLooks.consentAnswer, approvedAt: customerLooks.approvedAt, rejectedAt: customerLooks.rejectedAt });
  for (const row of updated) {
    await audit(db, { action: "look.revoke", lookId: row.id, source: input.source, after: { revokedAt: now.toISOString(), by: input.source } });
  }
  const wasPublic = updated.filter((row) => isLookPublic({ ...row, revokedAt: null })).length;
...
/** Painel: as últimas fotos, em qualquer estado (para a dona ver o que a Lia registrou e retirar). */
